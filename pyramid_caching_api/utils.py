...
class AttributeSafeObject(object):
    """object with lax attribute access ( returns an empty string '' when the attribute does not exist - good for templating ) Based on pylons."""
    ...
    def __getattr__(self, name):
        try:
            ...
            if DEBUG_ATTRIB_SAFE:
                log.debug("No attribute `%s` found in AttributeSafeObject instance, returning empty string" , name)
            return ''
    def keys(self):
