...
class CachedObject(pyramid_caching_api.utils.CachedData):
    ...
    def id_to_name(self, id, get_only=False):
        ...
        return self.regions_manager.regions[self.region_name].get_or_create(
            key, self._id_to_name
        )
    ...
    def ids_to_names(self, ids):
        ...
        check_ids = [
            i
            for i in api_results.keys()
            if (api_results[i] in pyramid_caching_api.api.CACHE_FAILS)
        ]
        if len(check_ids):
            print(
                "CachedObject.ids_to_names || POTENTIALLY EXPENSIVE FUNCTION || ids = %s"
                % check_ids
            )
            db_results = dict(
                (i, sample_data[i])
                if i in sample_data
                else (i, pyramid_caching_api.api.NO_VALUE)
                for i in check_ids
            )
...
class AdvancedCachedObject(pyramid_caching_api.utils.CachedData):
    ...
    def _get_by_id(self):
        (id,) = self.query_args
        print(
            "AdvancedCachedObject._get_by_id || A VERY EXPENSIVE FUNCTION || id = %s"
            % id
        )
        object = {
            "id": id,
            "title": "This is an Example Program",
    ...
    def get_by_id(self, id, create=True):
        ...
        if create:
            cached = self.regions_manager.regions[self.region_name].get_or_create(
                key, self._get_by_id
            )
        else:
...
r = FakeRequest()
dbSession = None
r.cachingApi = pyramid_caching_api.api.CachingApi(
    r, regions_manager=regions_manager, dbSessionReader=dbSession
)
...
if True:
    ...
    print(
        r.cachingApi.get(
            CachedObject, "ids_to_names", ((1, 2, 3, 5, 6, 100, 200, 300),)
        )
    )
